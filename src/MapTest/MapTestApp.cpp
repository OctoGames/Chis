...
MapTestApp::MapTestApp(const Ogre::String& appname) :
	Application(appname),
...
bool MapTestApp::keyPressed(const OgreBites::KeyboardEvent& evt)
{
...
void MapTestApp::setupScene(void)
{
	DotSceneLoader map;
	mMapNode = mSceneManager->getRootSceneNode()->createChildSceneNode("nMap");
	std::vector<std::string> vec = Ogre::ResourceGroupManager::getSingletonPtr()->getResourceGroups();
	...
	mMouseNode = mSceneManager->getSceneNode("nMap")->createChildSceneNode("nMouse");
